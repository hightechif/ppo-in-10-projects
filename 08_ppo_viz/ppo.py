...
class PPO:
    def __init__(
        ...
        obs_dim: int, 
        action_dim: int, 
        lr: float = 2.5e-4, # Standard baselines LR
        gamma: float = 0.99, 
        clip_ratio: float = 0.2,
        k_epochs: int = 4, # Standard
        batch_size: int = 64
    ) -> None:
        self.gamma = gamma
    ...
    def select_action(self, state: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        with torch.no_grad():
            ...
            # If state is 1D (single env), unsqueeze. If 2D (vector env), keep as is.
            if state_ten.dim() == 1:
                 state_ten = state_ten.unsqueeze(0)
    ...
    def update(
        ...
        """
        # Convert to tensors
        states_ten = torch.tensor(states, dtype=torch.float32)
        ...
        num_updates = self.k_epochs * (n_samples // self.batch_size)
